...
// Brand logo — uses the real DockCraft .png icon (the 256×256 entry extracted
// from dockcraft.ico). The white-circle background matches the sidebar's
// primary accent and gives the icon a solid silhouette at small sizes;
// `object-fit: contain` keeps the artwork centered when the user hasn't
// built the assets yet (the onerror swaps to a fallback letter mark).
const BRAND_LOGO = `<img class="brand-mark" src="/assets/static/images/dockcraft.png" alt="DockCraft" width="22" height="22" onerror="this.replace(document.createTextNode('A'))">`;
